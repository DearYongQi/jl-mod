...
public class MainActivity extends AppCompatActivity {
	...
	private static final String PREF_TEST_JAR_INSTALLED = "test_jar_installed";
	private static final String TEST_JAR_ASSET = "test.jar";
	...
	public void onCreate(@Nullable Bundle savedInstanceState) {
		...
		if (savedInstanceState == null) {
			...
			if (uri == null) {
				...
				if (!sp.getBoolean(PREF_TEST_JAR_INSTALLED, false)) {
					uri = installTestJarFromAssets();
				}
			}
	...
	private Uri installTestJarFromAssets() {
		try {
			File tempJar = new File(getCacheDir(), "test_bundled.jar");
			try (InputStream is = getAssets().open(TEST_JAR_ASSET);
			...
			PreferenceManager.getDefaultSharedPreferences(this)
					.edit().putBoolean(PREF_TEST_JAR_INSTALLED, true).apply();
			return Uri.fromFile(tempJar);
		} catch (IOException e) {
			e.printStackTrace();
			return null;
		}
